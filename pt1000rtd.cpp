...
int pt1000rtd::search_pt1000_list(uint32_t ohmsX100)
{
    int lower = 0 ;
    int upper = PT1000_TABLE_MAXIDX ;
    int mid = (lower + upper) / 2 ;

    do
    {
	uint16_t pt1000val = pgm_read_word_near(&Pt1000_table[mid]) ;
		
	if (pt100val == ohmsX100)
	{
	    break;
	}
	else if (pt1000val < ohmsX100)
	{
	    lower = mid + 1 ;
	}
	else
...
/**********************************************************************
** Function Name:	ohmsX1000_to_celsius
**
** Description:
...
**********************************************************************/
float pt1000rtd::ohmsX100_to_celsius (uint32_t ohmsX100)
{
    ...
    int index = search_pt1000_list(ohmsX100) ;
    ...
    R_lower = pgm_read_word_near(&Pt1000_table[index - 1]) ;
    R_upper = pgm_read_word_near(&Pt1000_table[index]) ;
	...
	uint16_t Rnext = pgm_read_word_near(&Pt1000_table[index + 1]) ;
...
float pt1000rtd::celsius (uint32_t ohmsX100)
{
    // clip underflow	
    if (ohmsX100 <= pgm_read_word_near(&Pt1000_table[0]))
    {
	// return min boundary temperature
	return((float) CELSIUS_MIN);
    }
    // clip overflow
    else if (ohmsX100 >= pgm_read_word_near(&Pt1000_table[PT100_TABLE_MAXIDX]))
    {
    ...
    {
	return(pt1000rtd::ohmsX100_to_celsius(ohmsX100)) ;
    }
}
...
float pt1000rtd::celsius (float rtd_ohms)
{
    // convert to unsigned short
    uint32_t ohmsX100 = (uint32_t) floor(rtd_ohms * 100.0) ;
    return pt1000rtd::celsius(ohmsX100) ;
}
...
float pt1000rtd::celsius_to_Pt1000ohms (float celsius)
{
    float Pt1000_ohms, T_delta ;
    uint16_t R_lower, R_delta, R_fraction ;
    int upper, lower ;

    if (celsius < (float)CELSIUS_MIN)
    {
	R_lower = Pt1000table[0] ;
	R_fraction = 0 ;
    }
    else if (celsius > (float)CELSIUS_MAX)
    {
	R_lower = Pt1000_table[PT100_TABLE_MAXIDX] ;
	R_fraction = 0 ;
    }
    else
    {
	...
	R_lower = Pt1000_table[lower] ;
	R_delta = Pt1000_table[upper] - R_lower ;
...
float pt1000rtd::celsius_cvd(float R_ohms)
{
...
float pt1000rtd::celsius_polynomial (float R_ohms)
{
    float rpoly, temp ;
    rpoly = R_ohms ;
